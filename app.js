...
function posZh(p){return {verb:'動詞',noun:'名詞',adjective:'形容詞',adverb:'副詞',pronoun:'代名詞',preposition:'前置詞',conjunction:'接続詞'}[p]||p}
function render(){
 const list=filtered();if(current>=list.length)current=0;
 $('#total').textContent=WORDS.length;
 $('#learned').textContent=Object.values(state).filter(Boolean).length;
 $('#remaining').textContent=WORDS.length-Object.values(state).filter(Boolean).length;
 $('#accuracy').textContent=round.total?Math.round(round.ok/round.total*100)+'%':'—';
 $('#counter').textContent=list.length?`${current+1} / ${list.length}`:'0 / 0';
 if(!list.length){
   $('#word').textContent='条件に一致する単語がありません';
   $('#backWord').textContent=''; return;
 }
 const x=list[current];
 flipped=false; $('#card').classList.remove('flipped');
 $('#badge').textContent=`${x.grade} · ${posZh(x.pos)}`;
 $('#word').textContent=x.word;
 $('#backWord').textContent=x.word;
 updateIPAForCurrentWord(x);
 $('#meaning').textContent=x.meaning_zh || '';
 $('#ja').textContent=x.meaning_ja || '';
 // 例文は言語ごとの専用フィールドからのみ表示
 $('#exEn').textContent=x.example_en || '';
 $('#exJa').textContent=x.example_ja || '';
 $('#exZh').textContent=x.example_zh || '';
}
...
$('#reviewBtn').onclick=()=>{reviewOnly=!reviewOnly;$('#reviewBtn').textContent=reviewOnly?'⭐ 復習模式':'⭐ 未習得';current=0;render()};
['grade','pos','search'].forEach(id=>$('#'+id).addEventListener('input',()=>{current=0;render()}));
$('#resetBtn').onclick=()=>{if(confirm('確認クリア所有学習记录吗？')){localStorage.removeItem('jhEnglishV2');location.reload()}};
...
// ===== V2.4.6 読み上げ：各データを各言語へ直接固定 =====
function chooseVoice(locale){
  if(!('speechSynthesis' in window)) return null;
  const vs=speechSynthesis.getVoices(), want=locale.toLowerCase(), lang=want.split('-')[0];
  return vs.find(v=>(v.lang||'').toLowerCase()===want)
      || vs.find(v=>(v.lang||'').toLowerCase().split('-')[0]===lang) || null;
}
function speakLocale(text,locale,rate=0.88){
  if(!text||!('speechSynthesis' in window))return;
  speechSynthesis.cancel();
  const u=new SpeechSynthesisUtterance(String(text));u.lang=locale;u.rate=rate;
  const v=chooseVoice(locale);if(v)u.voice=v;speechSynthesis.speak(u);
}
function currentWordData(){const list=filtered();return list.length?list[current]:null;}
function updateIPAForCurrentWord(x){const e=document.getElementById('ipaUS');if(e)e.textContent=x&&x.ipa_us?x.ipa_us:'—';}
document.getElementById('speakUS')?.addEventListener('click',e=>{e.stopPropagation();const x=currentWordData();if(x)speakLocale(x.word,'en-US',0.82);});
document.getElementById('speakMeaningZh')?.addEventListener('click',e=>{e.stopPropagation();const x=currentWordData();if(x)speakLocale(x.meaning_zh,'zh-CN');});
document.getElementById('speakExEn')?.addEventListener('click',e=>{e.stopPropagation();const x=currentWordData();if(x)speakLocale(x.example_en,'en-US');});
document.getElementById('speakExJa')?.addEventListener('click',e=>{e.stopPropagation();const x=currentWordData();if(x)speakLocale(x.example_ja,'ja-JP');});
document.getElementById('speakExZh')?.addEventListener('click',e=>{e.stopPropagation();const x=currentWordData();if(x)speakLocale(x.example_zh,'zh-CN');});
