...
function render(){
 ...
 if(!list.length){$('#word').textContent='条件に一致する単語がありません';$('#backWord').textContent='';return}
 const x=list[current];flipped=false;$('#card').classList.remove('flipped');$('#badge').textContent=`${x.grade} · ${posZh(x.pos)}`;$('#word').textContent=x.word;$('#backWord').textContent=x.word; updateIPAForCurrentWord(x);$('#meaning').textContent=x.meaning_zh;$('#ja').textContent=x.meaning_ja;$('#exEn').textContent=x.example_en;$('#exJa').textContent=x.example_ja;
}
...
$('#reviewBtn').onclick=()=>{reviewOnly=!reviewOnly;$('#reviewBtn').textContent=reviewOnly?'⭐ 復習モード':'⭐ 未習得';current=0;render()};
['grade','pos','search'].forEach(id=>$('#'+id).addEventListener('input',()=>{current=0;render()}));
$('#resetBtn').onclick=()=>{if(confirm('確認クリアすべての学習記録をリセットしますか？？')){localStorage.removeItem('jhEnglishV2');location.reload()}};
...
function speakWithLocale(text, locale){
  ...
  const exact = voices.find(v => v.lang && v.lang.toLowerCase() === locale.toLowerCase());
  const regional = voices.find(v => v.lang && v.lang.toLowerCase().startsWith(locale.slice(0,2).toLowerCase()));
  if (exact || regional) u.voice = exact || regional;
  window.speechSynthesis.speak(u);
}
document.addEventListener("DOMContentLoaded", () => {
  const getWord = () => {
    const el = document.getElementById("backWord");
    return el ? el.textContent.trim() : "";
  };
  const uk = document.getElementById("speakUK");
  const us = document.getElementById("speakUS");
  if (uk) uk.addEventListener("click", e => { e.stopPropagation(); speakWithLocale(getWord(), "en-GB"); });
  if (us) us.addEventListener("click", e => { e.stopPropagation(); speakWithLocale(getWord(), "en-US"); });
});

function updateIPAForCurrentWord(x){
  const uk=document.getElementById("ipaUK"), us=document.getElementById("ipaUS");
  if (!uk || !us) return;
  uk.textContent = (x && x.ipa_uk) ? x.ipa_uk : "—";
  us.textContent = (x && x.ipa_us) ? x.ipa_us : "—";
}

function updateExampleChinese(x){
  const z=document.getElementById("exZh");
  if(z) z.textContent=(x && x.example_zh) ? x.example_zh : "";
}

document.addEventListener("DOMContentLoaded",()=>{
  const speakText=(id,locale)=>{
    const el=document.getElementById(id);
    if(!el || !el.textContent.trim() || !("speechSynthesis" in window)) return;
    ...
    const exact=voices.find(v=>v.lang && v.lang.toLowerCase()===locale.toLowerCase());
    const same=voices.find(v=>v.lang && v.lang.toLowerCase().startsWith(locale.slice(0,2).toLowerCase()));
    if(exact||same) u.voice=exact||same;
    speechSynthesis.speak(u);
  };
  const bind=(btn,id,locale)=>{
    const b=document.getElementById(btn);
    if(b)b.addEventListener("click",e=>{e.stopPropagation();speakText(id,locale);});
  };
  bind("speakExEn","exEn","en-US");
  bind("speakExJa","exJa","ja-JP");
  bind("speakExZh","exZh","zh-CN");

  const bw=document.getElementById("backWord");
  if(bw && typeof WORDS!=="undefined"){
    const refreshZh=()=>{
      const w=bw.textContent.trim().toLowerCase();
      const x=WORDS.find(v=>String(v.word).toLowerCase()===w);
      updateExampleChinese(x);
    };
    new MutationObserver(refreshZh).observe(bw,{childList:true,subtree:true,characterData:true});
    refreshZh();
  }
});
